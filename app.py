...
with st.sidebar:
    ...
    f_v1 = st.file_uploader("THDR Vía 1", accept_multiple_files=True)
    f_v2 = st.file_uploader("THDR Vía 2", accept_multiple_files=True)
    f_umr = st.file_uploader("UMR / Odómetros", accept_multiple_files=True)
    f_seat_f = st.file_uploader("Energía SEAT", accept_multiple_files=True)
    f_bill_f = st.file_uploader("Facturación / PRMTE", accept_multiple_files=True)
                    ...
                    if 'FECHA' in df_p.columns:
                        ...
                        for _, r in df_p.dropna(subset=['DT']).iterrows():
                            if start_date <= r['DT'].date() <= end_date:
                                all_ops.append({"Fecha": r['DT'].normalize(), "Tipo Día": get_tipo_dia(r['DT']), "Odómetro [km]": parse_latam_number(r.get('ODO',0)), "Tren-Km [km]": parse_latam_number(r.get('TRENKM',0))})
    ...
    for f in f_bill_f:
        ...
        for sn in xl_b.sheet_names:
            ...
            if 'AÑO' in str(df_b.columns).upper():
                df_b['Timestamp'] = pd.to_datetime(df_b[['AÑO', 'MES', 'DIA', 'HORA']].astype(int).rename(columns={'AÑO':'year','MES':'month','DIA':'day','HORA':'hour'}))
                for _, r in df_b.iterrows():
                    v = parse_latam_number(r.get('Retiro_Energia_Activa (kWhD)', 0))
                    all_comp_full.append({"Fecha": r['Timestamp'].normalize(), "Hora": r['Timestamp'].hour, "Consumo": v, "Fuente": "PRMTE"})
                    if start_date <= r['Timestamp'].date() <= end_date: all_prmte_15.append({"Fecha": r['Timestamp'].normalize(), "Consumo": v})
...
with tabs[0]:
    if not df_ops.empty:
        ...
        st.plotly_chart(go.Figure(go.Scatter(x=df_ops['Fecha'], y=df_ops['Odómetro [km]'], name="Km")), use_container_width=True)
...
with tabs[2]:
    if not df_tr.empty: st.dataframe(df_tr, use_container_width=True)
    else: st.info("Sube archivos de odómetros diarios.")

with tabs[3]:
    if not df_ops.empty: st.dataframe(df_ops[['Fecha', 'E_Total', 'E_Tr', 'E_12']], use_container_width=True)

with tabs[4]:
    if all_comp_full:
        df_c = pd.DataFrame(all_comp_full).pivot_table(index="Hora", columns=pd.DataFrame(all_comp_full)['Fecha'].dt.year, values="Consumo", aggfunc='median')
        st.line_chart(df_c); st.dataframe(df_c)

with tabs[5]:
    if all_comp_full:
        df_r = pd.DataFrame(all_comp_full)
        h = st.selectbox("Hora Regresión", range(6))
        df_h = df_r[df_r['Hora']==h].sort_values("Fecha")
        if len(df_h)>2:
...
with tabs[6]:
    if all_comp_full:
        ...
        iqr = q3-q1
        st.dataframe(df_a[(df_a['Consumo']>q3+1.5*iqr) | (df_a['Consumo']<q1-1.5*iqr)])

with tabs[7]:
    st.write("#### Vía 1"); st.dataframe(df_thdr_v1, use_container_width=True)
    st.write("#### Vía 2"); st.dataframe(df_thdr_v2, use_container_width=True)

st.sidebar.download_button("Descargar Reporte", BytesIO().getvalue(), "Reporte.xlsx")
