...
def procesar_thdr_avanzado(file):
    try:
        ...
        # Deduplicar nombres para evitar error de PyArrow
        column_names, counts = [], {}
        ...
        # DETECCIÓN DINÁMICA DE ESTACIONES (Para servicios cortos)
                ...
                if 'SEAT' in sn_up and 'SER' in sn_up:
                    ...
                    for i in range(len(df_s)):
                        ...
                        if pd.notna(fs) and start_date <= fs.date() <= end_date:
                            ...
                            all_seat.append({"Fecha": fs.normalize(), "E_Total": tot, "E_Tr": tra, "E_12": k12})
    ...
    if not df_ops.empty:
        ...
        if not df_res_f.empty:
            ...
            with sub_tabs[0]:
                ...
                st.plotly_chart(go.Figure(go.Scatter(x=df_res_f['Fecha'], y=df_res_f['Odómetro [km]'], name="Odómetro")), use_container_width=True)
...
st.sidebar.download_button("📥 Reporte Completo", to_excel_consolidado(df_ops, df_tr, df_tr_acum, df_seat, df_p_d, df_f_d), "Reporte_EFE.xlsx")
