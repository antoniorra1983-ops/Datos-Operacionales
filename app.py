...
def procesar_thdr_avanzado(file):
    try:
        ...
        for i in range(len(h0)):
            ...
            if sub in ['Hora Llegada', 'Hora Salida']: column_names_raw.append(f"{base} ({sub})")
            else: column_names_raw.append(base)
        final_cols = []
        counts = {}
        ...
        def get_start(row):
            for c in cols_salida:
                ...
                if v is not None: return v, c.split('(')[0].strip()
            return None, None
        def get_end(row):
            for c in reversed(cols_llegada):
                ...
                if v is not None: return v, c.split('(')[0].strip()
            return None, None
        starts = df.apply(get_start, axis=1)
        ends = df.apply(get_end, axis=1)
        df['Hora_Salida_Real'] = starts.apply(lambda x: x[0])
        df['Origen'] = starts.apply(lambda x: x[1])
        df['Hora_Llegada_Real'] = ends.apply(lambda x: x[0])
        df['Destino'] = ends.apply(lambda x: x[1])
        def find_c(ks):
            for c in df.columns:
                            ...
                            if pd.notna(v) and start_date <= v.date() <= end_date:
                                for k in range(i+3, min(i+40, len(df_tr_raw))):
                                    ...
                                    if n.startswith(('M','XM')):
                                        ...
                                        all_tr_acum.append({"Tren":n, "Fecha":v.normalize(), "Valor":parse_latam_number(df_tr_raw.iloc[k,j])})
            ...
            for sn in xl_b.sheet_names:
                ...
                if 'AÑO' in str(df_b.columns).upper():
                    ...
                    for _, r in df_b.iterrows():
                        ...
                        if start_date <= r['TS'].date() <= end_date: all_prmte_15.append({"Fecha": r['TS'].normalize(), "Consumo": v})
...
with tabs[0]: # PESTAÑA RESUMEN (FILTROS COMPLETOS)
    if not df_ops.empty:
        ...
        if not df_res_f.empty:
            ...
            c1.metric("Odómetro Total", f"{df_res_f['Odómetro [km]'].sum():,.0f} km")
            c2.metric("Tren-Km Total", f"{df_res_f['Tren-Km [km]'].sum():,.0f} km")
...
with tabs[2]: # TRENES (DIARIO + ACUMULADO)
    ...
    if not df_tr_acum.empty:
        st.write("#### Odómetro Acumulado por Tren")
...
with tabs[5]: # REGRESIÓN
    if all_comp_full:
        ...
        if len(df_h) > 2:
            ...
            st.plotly_chart(go.Figure([go.Scatter(x=df_h['Fecha'], y=y, name="Real"), go.Scatter(x=df_h['Fecha'], y=m*x+b, name="Tendencia")]), use_container_width=True)
...
with tabs[6]: # ATÍPICOS
    if all_comp_full:
        ...
        iqr = q3 - q1
        st.error("Detección de anomalías en consumo horario")
        st.dataframe(df_at[(df_at['Consumo'] > q3+1.5*iqr) | (df_at['Consumo'] < q1-1.5*iqr)], use_container_width=True)
...
st.sidebar.divider()
st.sidebar.download_button("📥 Reporte Excel Completo", BytesIO().getvalue(), "Reporte_SGE.xlsx")
