...
# --- 1. CONFIGURACIÓN Y API DEL CORREDOR ---
st.set_page_config(page_title="Gestión de Energía - Dashboard SGE", layout="wide", page_icon="🚆")

# Clave de API integrada
API_KEY = "de25da707bfeb645ec2b488c4676af19" 

# Ciudades que componen el tramo operativo de EFE Valparaíso
CIUDADES_EFE = {
...
def obtener_clima_corredor():
    """Consulta el clima para cada ciudad del tramo Puerto-Limache."""
    resultados = {}
...
# --- 3. FUNCIONES THDR (CON FIX DE TRUTH VALUE ERROR) ---
...
def procesar_thdr_avanzado(file, start_date=None, end_date=None):
    try:
        ...
        c_serv, c_prog = buscar_columna(['Servicio', 'N° Servicio']), buscar_columna(['Hora_Prog', 'Programada'])
        ...
        # --- FIX TRUTH VALUE ERROR: Uso de .dt.date y comparación segura ---
        fecha_f = extraer_fecha_desde_nombre_archivo(file.name)
        df['Fecha_Op'] = pd.to_datetime(fecha_f if fecha_f else date.today())
...
with st.sidebar:
    ...
    f_bill_files = st.file_uploader("5. Facturación/PRMTE", type=["xlsx"], accept_multiple_files=True)
...
# --- 6. PROCESAMIENTO GENERAL (CON FIX DE TRUTH VALUE) ---
        ...
        try:
            ...
            for sn in xl.sheet_names:
                ...
                if any(k in sn_up for k in ['UMR', 'RESUMEN']):
                    ...
                    # FIX: any() sobre Series corregido con str()
                    ...
                    if h_r is not None:
                        ...
                        idx_f, idx_o = next((c for c in df_p.columns if 'FECHA' in c), None), next((c for c in df_p.columns if 'ODO' in c and 'ACUM' not in c), None)
                ...
                if 'ODO' in sn_up and 'KIL' in sn_up:
                    ...
                    for row_idx, s_dt in headers_found:
                        for k in range(row_idx+3, min(row_idx+40, len(df_tr_raw))):
                            n_tr = str(df_tr_raw.iloc[k, 0]).strip().upper()
                            if re.match(r'^(M|XM)', n_tr):
                                val_km = parse_latam_number(df_tr_raw.iloc[k, 1]) # Ejemplo simplificado
                                all_tr.append({"Tren": n_tr, "Fecha": s_dt.normalize(), "Valor": val_km})
...
with tabs[5]:
    ...
    def mostrar_thdr_ordenada(df, titulo):
        ...
        for e in estaciones:
            if f"{e}_llegada_fmt" in df.columns: f_cols.append(f"{e}_llegada_fmt")
            if f"{e}_salida_fmt" in df.columns: f_cols.append(f"{e}_salida_fmt")
