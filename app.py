...
def to_excel_consolidado(df_ops, df_tr, df_seat):
    output = BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        if not df_seat.empty: df_seat.to_excel(writer, index=False, sheet_name='Energía_SEAT')
    return output.getvalue()

# --- 3. FUNCIONES THDR (SIN TOCAR NADA) ---
...
# --- 6. PROCESAMIENTO REPARADO ---
if any([f_v1, f_v2, f_umr, f_seat_files, f_bill_files]):
    # A. PROCESAR UMR (REPARADO: Búsqueda agresiva de cabeceras)
    if f_umr:
        for f in f_umr:
            try:
                xl = pd.ExcelFile(f)
                for sn in xl.sheet_names:
                    ...
                    for i in range(min(100, len(df_raw))):
                        ...
                        # Detección flexible: Fecha, Odómetro, Tren-Km
                        if any(k in fila_str for k in ['FECHA', 'ODO', 'TREN-KM', 'KILOM']):
                            h_r = i; break
                    
                    if h_r is not None:
                        df_p = pd.read_excel(f, sheet_name=sn, header=h_r)
                        # Limpieza de nombres de columna (quitar tildes y caracteres raros)
                        ...
                        if c_f and c_o:
                            ...
                            df_p = df_p[mask].dropna(subset=['_dt'])
                    ...
                    for i in range(len(df_tr_raw)-2):
                        for j in range(1, len(df_tr_raw.columns)):
                            ...
                            if pd.notna(v_f) and start_date <= v_f.date() <= end_date:
                                for k in range(i+3, min(i+50, len(df_tr_raw))):
    ...
    if all_ops:
        ...
        if not df_em.empty:
            ...
            if 'E_Tr' not in df_ops.columns and 'E_Total' in df_ops.columns:
                df_ops['E_Tr'] = df_ops['E_Total'] * 0.85 # Factor basal estimado
...
with tabs[0]: # PESTAÑA RESUMEN (RESTAURADA)
    if not df_ops.empty:
        ...
        if not df_rf.empty:
            ...
            m2.metric("Tren-Km Total", f"{df_rf['Tren-Km [km]'].sum():,.1f} km")
            m3.metric("IDE Promedio", f"{df_rf['IDE (kWh/km)'].mean():.4f} kWh/km")
            ...
            df_jor = df_rf.groupby("Tipo Día")["IDE (kWh/km)"].mean().reindex(ORDEN_TIPO_DIA).dropna()
            st.bar_chart(df_jor)
        else: st.warning("Ajusta los filtros para ver los resultados.")
...
with tabs[5]: # REGRESIÓN
    if all_comp_full:
        ...
        if len(df_reg) > 2:
            x_reg = np.arange(len(df_reg))
            y_reg = df_reg['Consumo'].values
            m, n = np.polyfit(x_reg, y_reg, 1)
...
# REPORTE
st.sidebar.download_button("📥 Excel Completo", to_excel_consolidado(df_ops, pd.DataFrame(all_tr), pd.DataFrame(all_seat)), "EFE_Reporte.xlsx")
