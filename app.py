...
st.markdown("""
    <style>
    .stMetric { background-color: #ffffff; padding: 20px; border-radius: 10px; border-left: 5px solid #005195; }
    </style>
...
# --- 3. PROCESAMIENTO THDR (INALTERADO) ---
...
if any([f_v1, f_v2, f_umr, f_seat_files, f_bill_files]):
    ...
    if all_ops:
        ...
        # BLINDAJE: Columnas vacías por defecto para evitar KeyError
...
with tabs[0]: # RESUMEN BLINDADO
    if not df_ops.empty:
        anios = sorted(df_ops['Fecha'].dt.year.unique())
        f_a = st.multiselect("Año", anios, default=anios)
        df_rf = df_ops[df_ops['Fecha'].dt.year.isin(f_a)]
        if not df_rf.empty:
            col1, col2, col3 = st.columns(3)
            # Verificación de existencia de columnas antes de operar
            v_odo = df_rf['Odómetro [km]'].sum() if 'Odómetro [km]' in df_rf.columns else 0
            v_tkm = df_rf['Tren-Km [km]'].sum() if 'Tren-Km [km]' in df_rf.columns else 0
            v_ide = df_rf['IDE (kWh/km)'].mean() if 'IDE (kWh/km)' in df_rf.columns else 0
            col1.metric("Odómetro", f"{v_odo:,.1f} km")
            col2.metric("Tren-Km", f"{v_tkm:,.1f} km")
            col3.metric("IDE Prom", f"{v_ide:.4f}")
