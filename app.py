...
def to_excel_consolidado(data_dict):
    output = BytesIO()
...
# --- 3. MOTOR DE PROCESAMIENTO (TU LÓGICA DE MAÑANA) ---

def procesar_archivos(f_umr, f_seat_files, f_bill_files, start_date, end_date):
    ...
    return all_ops, all_tr, all_tr_acum, all_seat, all_prmte_15, all_fact_h, all_comp_full

# --- 4. FUNCIONES DE RENDERIZADO (UI) ---

def render_tab_resumen(df_ops):
...
def render_tab_trenes(df_tr, df_tr_acum):
    st.header("🚆 Control de Kilometraje Trenes")
...
def render_tab_regresion(all_comp_full):
    ...
    if all_comp_full:
        ...
        if len(df_pl) > 1:
            ...
            if len(df_norm) > 1:
                ...
                st.info(f"Carga basal inicial: {n:.2f} kWh.")
                m_reg = {"Ecuación": f"Consumo = {m:.4f}x + {n:.2f}", "R2": f"{r2:.4f}"}
                st.download_button("📥 Descargar Regresión (PPTX)", to_pptx(f"Regresión Hora {f_rh}", df_norm[['Fecha','Consumo Horario [kWh]']], m_reg), "EFE_Regresion.pptx")
...
def main():
    ...
    # Inicializar Outliers
    if 'outliers' not in st.session_state: st.session_state.outliers = pd.DataFrame()

    # Procesamiento
    all_ops, all_tr, all_tr_acum, all_seat, all_prmte_15, all_fact_h, all_comp_full = procesar_archivos(f_umr, f_seat_files, f_bill_files, start_date, end_date)
    
    # Construcción de DataFrames (Jerarquía)
    df_ops, df_tr, df_tr_acum, df_seat, df_p_d, df_f_d = pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    ...
    if any([all_ops, all_tr, all_tr_acum, all_seat, all_prmte_15, all_fact_h]):
        ...
        # Lógica de Master de Energía (Simplificada para el ejemplo)
        ...
        # RENDERIZADO DE TABS
        tabs = st.tabs(["📊 Resumen", "📑 Operaciones", "📑 Trenes", "⚡ Energía", "⚖️ Comparación", "📈 Regresión Nocturna", "🚨 Datos Atípicos"])
        
        with tabs[0]: render_tab_resumen(df_ops)
        with tabs[1]: st.dataframe(get_filtros(df_ops, "ops"), use_container_width=True)
        with tabs[2]: render_tab_trenes(df_tr, df_tr_acum)
        with tabs[3]: st.write("⚡ Datos SEAT/PRMTE/Factura cargados."); st.dataframe(df_seat)
        with tabs[5]: render_tab_regresion(all_comp_full)
        with tabs[6]: 
            if not st.session_state.outliers.empty:
                st.error("🚨 Anomalías detectadas"); st.dataframe(st.session_state.outliers)
            else: st.success("✅ Sin anomalías.")

        # Botón Excel Global
        st.sidebar.download_button("📥 Reporte Excel Completo", to_excel_consolidado({'Ops':df_ops, 'Trenes':df_tr, 'SEAT':df_seat}), "Reporte_EFE_SGE.xlsx")
    else:
