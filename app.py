...
def to_excel_consolidado(df_ops, df_tr, df_seat, df_energy):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        if not df_energy.empty: df_energy.to_excel(writer, index=False, sheet_name='Energia_Consolidada')
    return output.getvalue()

# --- 3. FUNCIONES THDR (INALTERADAS) ---
...
# --- 6. PROCESAMIENTO DE DATOS (REPARADO Y SIN ERRORES DE SINTAXIS) ---
if any([f_v1, f_v2, f_umr, f_seat_files, f_bill_files]):
    
    # A. PROCESAR UMR Y TRENES (Todo en un solo flujo para evitar SyntaxErrors)
    if f_umr:
        for f in f_umr:
            try:
                xl = pd.ExcelFile(f)
                for sn in xl.sheet_names:
                    ...
                    # 1. Buscar Datos Operacionales (all_ops)
                    h_r = None
                    for i in range(min(100, len(df_raw))):
                        fila_str = " ".join([str(x).upper() for x in df_raw.iloc[i].tolist()])
                        if any(k in fila_str for k in ['FECHA', 'ODO', 'TREN-KM', 'KILOM']):
                            h_r = i; break
                    ...
                    if h_r is not None:
                        ...
                        if c_f and c_o:
                            ...
                            for _, r in df_filt.iterrows():
                                all_ops.append({
                                    "Fecha": r['_dt'], "Tipo Día": get_tipo_dia(r['_dt']),
                                    "Odómetro [km]": parse_latam_number(r[c_o]),
                                    "Tren-Km [km]": parse_latam_number(r[c_t]) if c_t else 0.0
                                })

                    # 2. Buscar Kms por Tren (all_tr)
                    if any(k in sn_up for k in ['KIL', 'ODO']):
                                ...
                                if pd.notna(v_f) and start_date <= v_f.date() <= end_date:
                                    for k in range(i+3, min(i+50, len(df_raw))):
                                        ...
                                        if re.match(r'^(M|XM)', tren):
                                            all_tr.append({"Tren": tren, "Fecha": v_f.normalize(), "Valor": parse_latam_number(df_raw.iloc[k, j])})
            ...
            try:
                ...
                for i in range(len(df_s)):
                    ...
                    if pd.notna(fs) and start_date <= fs.date() <= end_date:
                        all_seat.append({
                            "Fecha": fs.normalize(), "E_Total": parse_latam_number(df_s.iloc[i, 3]),
                            "E_Tr": parse_latam_number(df_s.iloc[i, 5]), "E_12": parse_latam_number(df_s.iloc[i, 7]),
                            "% Tr": (parse_latam_number(df_s.iloc[i, 5])/parse_latam_number(df_s.iloc[i, 3])*100 if parse_latam_number(df_s.iloc[i, 3])>0 else 0)
                        })
            except: continue

    # C. PROCESAR FACTURACIÓN / PRMTE
    if f_bill_files:
        for f in f_bill_files:
            try:
                ...
                for sn in xl.sheet_names:
                    sn_up = sn.upper()
                    if 'FACT' in sn_up:
                        df_f = pd.read_excel(f, sheet_name=sn); df_f.columns = ['Fch', 'Val']
                        df_f['dt'] = pd.to_datetime(df_f['Fch'], errors='coerce')
                        for _, r in df_f.dropna(subset=['dt']).iterrows():
                            all_comp_full.append({"Fecha": r['dt'].normalize(), "Hora": r['dt'].hour, "Consumo": abs(parse_latam_number(r['Val'])), "Fuente": "Factura", "Año": r['dt'].year, "Tipo Día": get_tipo_dia(r['dt'])})
                    if 'PRMTE' in sn_up:
                        ...
                        if h_idx is not None:
                            ...
                            for _, r in df_pd.iterrows():
                                all_comp_full.append({"Fecha": r['ts'].normalize(), "Hora": r['ts'].hour, "Consumo": parse_latam_number(r.get('Retiro_Energia_Activa (kWhD)', 0)), "Fuente": "PRMTE", "Año": r['ts'].year, "Tipo Día": get_tipo_dia(r['ts'])})
            except: continue

    # --- D. CONSOLIDACIÓN FINAL (PARA EL RESUMEN) ---
    if all_ops:
        ...
        if all_comp_full:
            ...
            for fnt in ["Factura", "PRMTE"]:
                ...
                if not df_fnt.empty:
                    df_em = pd.concat([df_em if not df_em.empty else pd.DataFrame(), df_fnt[["Fecha", "E_Total"]]]).drop_duplicates(subset=["Fecha"], keep="last")
        ...
        if not df_em.empty:
            ...
            if 'E_Tr' not in df_ops.columns and 'E_Total' in df_ops.columns:
                df_ops['E_Tr'] = df_ops['E_Total'] * 0.85
            df_ops['IDE (kWh/km)'] = df_ops.apply(lambda r: r.get('E_Tr', 0) / r['Odómetro [km]'] if r['Odómetro [km]'] > 0 else 0, axis=1)
...
with tabs[0]: # RESUMEN
    if not df_ops.empty:
        ...
        if not df_rf.empty:
            m1, m2, m3 = st.columns(3); m1.metric("Odómetro", f"{df_rf['Odómetro [km]'].sum():,.1f} km"); m2.metric("Tren-Km", f"{df_rf['Tren-Km [km]'].sum():,.1f} km"); m3.metric("IDE Prom", f"{df_rf['IDE (kWh/km)'].mean():.4f}")
            st.plotly_chart(go.Figure(data=[go.Bar(x=df_rf['Fecha'], y=df_rf['Odómetro [km]'], marker_color="#005195")], layout=go.Layout(title="Evolución Odómetro")), use_container_width=True)
            st.write("### ⚡ Eficiencia por Tipo de Día")
            st.bar_chart(df_rf.groupby("Tipo Día")["IDE (kWh/km)"].mean().reindex(ORDEN_TIPO_DIA).dropna())
    else: st.info("Sube archivos (UMR) para ver el resumen.")
...
with tabs[3]: # ENERGÍA
    if not df_seat.empty or all_seat:
        df_se = pd.DataFrame(all_seat) if all_seat else df_seat
        st.dataframe(make_columns_unique(df_se).style.format({'E_Total': "{:,.0f}", 'E_Tr': "{:,.0f}"}))

with tabs[4]: # COMPARACIÓN
    if all_comp_full:
        df_c = pd.DataFrame(all_comp_full).groupby(['Año', 'Tipo Día', 'Hora'])['Consumo'].median().reset_index()
        fig_c = px.line(df_c, x="Hora", y="Consumo", color="Año", line_dash="Tipo Día", title="Perfil Horario Mediano")
...
with tabs[6]: # ATÍPICOS
    st.info("Detección de anomalías basada en consumo basal (00:00 - 05:00).")

with tabs[7]: # THDR
    st.header("📋 Datos THDR")
    if 'df_thdr_v1' in locals() and not df_thdr_v1.empty:
        st.subheader("Vía 1 (Puerto -> Limache)"); st.dataframe(make_columns_unique(df_thdr_v1))
    if 'df_thdr_v2' in locals() and not df_thdr_v2.empty:
        st.subheader("Vía 2 (Limache -> Puerto)"); st.dataframe(make_columns_unique(df_thdr_v2))

# REPORTE
st.sidebar.download_button("📥 Excel Completo", to_excel_consolidado(df_ops, pd.DataFrame(all_tr), pd.DataFrame(all_seat), df_energy_master), "Reporte_SGE_Total.xlsx")
