...
# --- 3. PROCESAMIENTO THDR (SOLUCIÓN ERROR AMBIGÜEDAD) ---
...
# --- 6. PROCESAMIENTO (SOLUCIÓN TRUTH VALUE) ---
        ...
        try:
            ...
            for sn in xl.sheet_names:
                ...
                if any(k in sn_up for k in ['UMR', 'RESUMEN']):
                    ...
                    # SOLUCIÓN: Convertir a lista para búsqueda escalar segura
                    h_r = None
                    for i in range(min(100, len(df_raw))):
                        fila_lista = df_raw.iloc[i].astype(str).tolist()
                        fila_texto = " ".join(fila_lista).upper()
...
# ================== PESTAÑA RESUMEN (SOLUCIÓN STREAMLITAPIEXCEPTION) ==================
with tabs[0]:
    if not df_ops.empty:
        ...
        # FIX: Filtrar default para asegurar que existan en las opciones actuales
        ...
        f_ano = c1.multiselect("Año", anios, default=def_anios or anios, key="f_ano")
        f_mes = c2.multiselect("Mes", meses, default=def_meses or meses, key="f_mes")
        ...
        if not df_res_f.empty:
            ...
            m1.metric("Odómetro Total", f"{df_res_f['Odómetro [km]'].sum():,.1f} km")
            m2.metric("Tren-Km Total", f"{df_res_f['Tren-Km [km]'].sum():,.1f} km")
...
with tabs[7]:
    ...
    def mostrar_thdr(df, titulo):
        if df.empty: return st.info(f"Sin datos para {titulo}")
        cols_fmt = [c for c in df.columns if str(c).endswith('_fmt')]
        df_d = df[['Fecha_Op', 'Servicio', 'Unidad'] + cols_fmt].copy()
        st.dataframe(df_d, use_container_width=True)
    
    mostrar_thdr(df_thdr_v1, "Vía 1 (Puerto → Limache)")
    mostrar_thdr(df_thdr_v2, "Vía 2 (Limache → Puerto)")

# --- 8. EXPORTACIÓN ---
st.sidebar.download_button("📥 Reporte Completo", to_excel_consolidado(df_ops, df_tr, df_tr_acum, df_seat, df_p_d, pd.DataFrame(all_prmte_15), pd.DataFrame(all_fact_h), df_f_d), "EFE_Reporte_Final.xlsx")
