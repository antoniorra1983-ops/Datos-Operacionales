...
# --- 4. MOTOR DE PROCESAMIENTO (RESTAURADO TODA LA LÓGICA) ---
    ...
    try:
        ...
        for sn in xl.sheet_names:
            ...
            if any(k in su for k in ['UMR', 'RESUMEN']):
                ...
                h = next((i for i in range(min(100, len(df))) if any(k in str(df.iloc[i]).upper() for k in ['ODO', 'FECHA'])), None)
                if h is not None:
                        ...
                        if pd.notna(dt) and start_date <= dt.date() <= end_date:
                            for k in range(i+3, min(i+40, len(df_r))):
                                ...
                                if re.match(r'^(M|XM)', tr):
                                    ...
                                    if any(k in su for k in ['ACUM', 'LECTURA', 'TOTAL']): all_tr_acum.append(item)
            ...
            # Lógica PRMTE / Facturación
            if any(k in su for k in ['PRMTE', 'FACTURA', 'MEDIDA', 'CONSUMO']):
                df_f = pd.read_excel(f, sheet_name=sn, header=None)
                hi = next((i for i in range(len(df_f)) if any(k in str(df_f.iloc[i]).upper() for k in ['AÑO', 'FECHAHORA', 'INTERVALO'])), None)
                if hi is not None:
                    ...
                    elif any(k in df_d.columns for k in ['FECHAHORA', 'TS', 'FECHA']): # Caso Factura
                        df_d.columns = ['TS', 'Val']
...
# Triangulación de Energía para el Resumen (Prioridad Factura > PRMTE > SEAT)
df_m = pd.DataFrame()
...
if not df_ops.empty and not df_m.empty:
    df_ops = pd.merge(df_ops, df_m, on="Fecha", how="left")
...
if not df_ops.empty or not df_seat.empty or all_comp_full:
    ...
    with tabs[0]: # --- RESUMEN ---
        ...
        if not df_ops.empty:
            c1, c2 = st.columns(2)
            f_ano = c1.multiselect("Año", sorted(df_ops['Fecha'].dt.year.unique()), default=sorted(df_ops['Fecha'].dt.year.unique()), key="res_a")
            ...
            if not df_f.empty:
                ...
                st.columns(3)[0].metric("Odómetro Total", f"{to:,.1f} km")
                st.columns(3)[1].metric("Tren-Km Total", f"{tk:,.1f} km")
                st.columns(3)[2].metric("UMR Global", f"{(tk/to*100) if to>0 else 0:.2f}%")
    ...
    with tabs[1]: # --- OPERACIONES ---
        st.header("📑 Datos Operacionales Completos")
    ...
    with tabs[3]: # --- ENERGÍA CRUDA ---
        st.header("⚡ Datos Base de Energía (Auditoría)")
    ...
    with tabs[4]: # --- COMPARACIÓN HORARIA ---
        if all_comp_full:
            st.header("⚖️ Comparativa Horaria Anual")
    ...
    with tabs[5]: # --- REGRESIÓN NOCTURNA ---
        ...
        if all_comp_full:
            ...
            if not df_reg.empty:
                ...
                f_rh = c2.selectbox("Hora", range(6), key="reg_h")
    ...
    st.info("👋 Sube los archivos en el panel lateral para activar el dashboard.")
