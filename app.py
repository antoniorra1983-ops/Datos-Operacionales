...
def procesar_thdr_eficiente(file, start_date, end_date):
    try:
        ...
        # 1. Extraer Fecha desde A1 (Celda 0,0)
        # Soporta 10126 (1-ene-26) o 010126 (01-ene-26)
        fecha_raw = str(df_raw.iloc[0, 0]).strip().split('.')[0]
        if len(fecha_raw) == 5: fecha_raw = "0" + fecha_raw
        dia, mes, anio = int(fecha_raw[:2]), int(fecha_raw[2:4]), 2000 + int(fecha_raw[4:])
        ...
        # Fila 0 tiene estaciones (con la fecha en A1 que debemos ignorar)
        ...
        # 3. Datos en Fila 6 (Index 5) saltando las 3 vacías
        df = df_raw.iloc[5:].copy()
        df.columns = cols
        df = make_columns_unique(df).dropna(how='all', axis=0)
        
        # Procesar minutos y kms
        ...
        # Referencia para tablas de frecuencia
                ...
                for sn in xl.sheet_names:
                    if 'FACT' in sn.upper():
                        ...
                        for _, r in df_f.dropna(subset=['dt']).iterrows(): 
                            ...
                            all_fact_full.append({"Fecha": r['dt'].normalize(), "Hora": r['dt'].hour, "Consumo [kWh]": v})
                            all_comp_full.append({"Fecha": r['dt'].normalize(), "Hora": r['dt'].hour, "Consumo": v, "Fuente": "Factura"})
                    if 'PRMTE' in sn.upper():
                        ...
                        for _, r in df_pd.iterrows(): 
                            ...
                            all_prmte_full.append({"Fecha": r['ts'].normalize(), "Hora": r['ts'].hour, "Energía Activa [kWh]": v})
                            all_comp_full.append({"Fecha": r['ts'].normalize(), "Hora": r['ts'].hour, "Consumo": v, "Fuente": "PRMTE"})
            except: pass

    # CONSOLIDACIÓN OPERACIONES E IDE
    if all_ops:
...
with tabs[1]: # OPERACIONES (RECUPERADO)
    if not df_ops.empty:
...
with tabs[3]: # ENERGÍA (RECUPERADO CON 3 SUB-PESTAÑAS)
    e_tabs = st.tabs(["🔹 SEAT", "🔹 PRMTE", "🔹 Facturación"])
    with e_tabs[0]:
        if all_seat: st.dataframe(pd.DataFrame(all_seat).style.format({'Total [kWh]':"{:,.0f}"}))
    with e_tabs[1]:
        if all_prmte_full:
            df_p = pd.DataFrame(all_prmte_full)
            st.write("#### Consumo Diario PRMTE")
            st.dataframe(df_p.groupby("Fecha")["Energía Activa [kWh]"].sum().reset_index().style.format({'Energía Activa [kWh]':"{:,.2f}"}))
            st.write("#### Detalle")
            st.dataframe(df_p.head(100))
    with e_tabs[2]:
        if all_fact_full:
            df_f = pd.DataFrame(all_fact_full)
            st.write("#### Consumo Diario Factura")
            st.dataframe(df_f.groupby("Fecha")["Consumo [kWh]"].sum().reset_index().style.format({'Consumo [kWh]':"{:,.2f}"}))

with tabs[7]: # THDR (CORREGIDO)
    ...
    else: st.info("Sube la THDR y verifica el rango de fechas.")
